...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("OK But Try Later😂", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await client.send_message(
                   ...
                   text =f'<b> > Title : {title}\n> File Size : {size}\n\n<u>#Note : Idu Telegram File dhaan</u> Dulink Ula Poii edukanum ‼️ Epdi Edukuradhu nu therilaya? Killa Vid irukum Paruga</b>',reply_markup=InlineKeyboardMarkup(
            [
                [
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b><i>Rᴇǫᴜᴇsᴛᴇᴅ Mᴏᴠɪᴇ Nᴀᴍᴇ : <code>{search}</code>\n\n  <u>Note : Files Sended To PM Check In Private </u></b></i>"
    if imdb and imdb.get('poster'):
        
        try:
            trvpn4 = await message.reply_photo(photo=imdb.get('poster'), caption=cap[:1024],
                                      reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(10100)
            await trvpn4.delete()
...
async def advantage_spell_chok(msg):
    ...
    krk = await msg.reply("I couldn't find anything related to that\nDid you mean any one of these?",
                    reply_markup=InlineKeyboardMarkup(btn))
